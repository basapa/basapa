...
/*virtual override*/class CCContentsNavigationInfoView extends CCNavigationInfoView
{
	//JS PHP AGREEMENTS...
	//...JS PHP AGREEMENTS

	//IDS...
	private const _INFO_ID								= 'contents_navigation_info_id';
	//...IDS

	//CSS...
	private const _CSS_INFO								= 'css_contents_navigation_info';
	//...CSS
	...
	//TRANSLATIONS...
	private const _INFO_XML_NODE_ID						= 'content_navigation_info';
	private const _CONTENT_LABELLIST_XML_NODE_ID		= 'content_labellist';
	//...TRANSLATIONS
	...
	/*virtual override*/protected function __drawBodyContent():void
	{
		...
		$lParams_a_arr	= [	CCSimpleLabellistComponentView::i_PARAM_ID_ITEMS_PARAMS						=> $this->_generateItemsParams(),
							CCSimpleLabellistComponentView::i_PARAM_ID_OPT_COMPONENT_CLASSES			=> CCContentsNavigationInfoView::_LABELLIST_COMPONENT_CLASSES,
							CCSimpleLabellistComponentView::i_PARAM_ID_OPT_COMPONENT_ELEMENT_CLASSES	=> CCContentsNavigationInfoView::_LABELLIST_COMPONENT_ELEMENT_CLASSES];
	...
	private function _generateItemsParams():array
	{
		...
		$lPrevItemValue_str = '';
		$lPrevItemState_int = CCLabellistComponentItem::i_ITEM_STATE_DISABLED;
		$lNextItemValue_str = '';
		$lNextItemState_int = CCLabellistComponentItem::i_ITEM_STATE_DISABLED;
		...
		for ($i = 0; $i < $lCount_int; $i++)
		{
			...
			else
			{
				...
				if ($i === 0)
				{
					$lPrevItemValue_str = $lItemValue_str;
					$lPrevItemState_int = $lItemState_int === CCLabellistComponentItem::i_ITEM_STATE_SELECTED ? CCLabellistComponentItem::i_ITEM_STATE_DISABLED : CCLabellistComponentItem::i_ITEM_STATE_ENABLED;
				}
				else if ($i === $lCount_int - 1)
				{
					$lNextItemValue_str = $lItemValue_str;
					$lNextItemState_int = $lItemState_int === CCLabellistComponentItem::i_ITEM_STATE_SELECTED ? CCLabellistComponentItem::i_ITEM_STATE_DISABLED : CCLabellistComponentItem::i_ITEM_STATE_ENABLED;
				}
			}
		...
		if ($lMaxContentPagesCount_int > $lMinNavigationItemsCount_int)
		{
			$lLabellistTranslationData_clss = $this->__getTranslationXMLLabellistComponentNode(self::_CONTENT_LABELLIST_XML_NODE_ID);

			$lPrevItemName_str = $lLabellistTranslationData_clss->i_getItemsData()->i_getItemNameByItemId("prev");
			$lNextItemName_str = $lLabellistTranslationData_clss->i_getItemsData()->i_getItemNameByItemId("next");

			CArrayTools::i_insertAtStart($lItemsParams_a_arr_arr, $this->__generateItemParams($lItemIndex_int + 1, $lPrevItemName_str, '', $lPrevItemValue_str, $lPrevItemState_int, 'css_prev'));

			$lItemsParams_a_arr_arr[] = $this->__generateItemParams($lItemIndex_int + 2, $lNextItemName_str, '', $lNextItemValue_str, $lNextItemState_int, 'css_next');
		}
	...
	//TRANSLATIONS...
	/*virtual override*/protected function __getInfoNodeName():string
	{
